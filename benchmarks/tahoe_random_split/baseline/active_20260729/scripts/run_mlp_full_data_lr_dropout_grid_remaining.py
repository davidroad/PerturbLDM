#!/usr/bin/env python3
"""Full-data MLP learning-rate stage without OOD/test access.

The completed learning-rate 1e-4, dropout 0 result is read only. The two remaining
learning-rate candidates are trained with dropout 0 on the same 29,277 conditions
and checked every epoch on the same 3,252-condition validation set. The imported formal trainer provides
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=RUN / "sensitivity/mlp_full_data_learning_rate_stage_20260727",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    common = {
        "analysis": "compact full-data learning-rate selection at dropout 0",
    ...
    rows = []
    require(DROPOUTS == (0.0,), "learning-rate stage must keep dropout fixed at 0")
    for learning_rate in LEARNING_RATES:
        dropout = 0.0
        label = config_name(learning_rate, dropout)
        candidate_root = output_root / label
        row = run_training_stage(
            stage_name=f"full_data_learning_rate_selection__{label}",
            x_train=x_train,
            y_train=y_train,
            x_valid=x_valid,
            y_valid=y_valid,
        ...
        require(float(stage["learning_rate"]) == float(learning_rate), f"{label} learning rate differs")
        require(float(stage["dropout"]) == 0.0, f"{label} dropout differs")
        if not args.smoke_test:
            for key in (
                "batch_size",
    ...
    table = pd.DataFrame(
        [row_for_comparison(config_name(LEARNING_RATE, 0.0), formal_row, validation_hash), *rows]
    ).sort_values(["validation_delta_mse", "learning_rate"], kind="stable").reset_index(drop=True)
    require(len(table) == 3, "learning-rate comparison must contain three candidates")
    require(set(table["learning_rate"].astype(float)) == {1e-4, 5e-4, 1e-3}, "learning-rate set changed")
    require(table["dropout"].astype(float).eq(0.0).all(), "dropout changed during learning-rate stage")
    require(not table.duplicated(["learning_rate", "dropout"]).any(), "duplicate candidate")
    ...
    best = table.iloc[0]
    comparison_path = output_root / "full_data_learning_rate_comparison.csv"
    ...
    summary = {
        "status": "FULL_DATA_LEARNING_RATE_STAGE_OK",
        "completed_utc": datetime.now(timezone.utc).isoformat(),
        "selection_metric": "3252-condition formal-validation delta-expression MSE",
        "selection_mode": "strict argmin across three learning rates at dropout 0",
        "strict_argmin_learning_rate": float(best["learning_rate"]),
        "fixed_dropout": 0.0,
    ...
    write_json_atomic(output_root / "full_data_learning_rate_comparison.json", summary)
