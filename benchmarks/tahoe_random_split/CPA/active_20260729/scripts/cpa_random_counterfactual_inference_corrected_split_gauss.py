...
TAHOE_TEST_H5AD = Path(os.environ.get("TAHOE_TEST_H5AD", str(BENCHMARK_ROOT / "external_inputs/tahoe/test_adata_processed.h5ad")))
TAHOE_CONTROL_H5AD = Path(os.environ.get("TAHOE_CONTROL_H5AD", str(BENCHMARK_ROOT / "external_inputs/tahoe/control_adata_processed.h5ad")))
CPA_MODEL_PATH = Path(os.environ.get("CPA_MODEL_PATH", "random_gauss_result/models/cpa_global_model_gauss.pth"))
...
def setup_logging():
    ...
    log_dir = "./results/random_inference_full_gauss/logs"
...
def load_completed_cell_line(cell_line, conditions, var_names):
    """Reuse a complete, internally consistent cell-line shard after a retry."""
    safe_cell_line = cell_line.replace("-", "_")
    inference_path = (
        f"./results/random_inference_full_gauss/by_cell_line/"
        f"cpa_inference_{safe_cell_line}.h5ad"
    )
    metrics_path = (
        f"./results/random_inference_full_gauss/by_cell_line/"
        f"metrics_{safe_cell_line}.json"
    )
    if not (os.path.isfile(inference_path) and os.path.isfile(metrics_path)):
        return None

    expected = {
        f"{line}_{drug}_{dose_str}"
        for line, drug, dose_str in conditions
    }
    try:
        with open(metrics_path) as handle:
            payload = json.load(handle)
        metrics = payload["metrics_by_condition"]
        if set(metrics) != expected:
            return None

        backed = sc.read_h5ad(inference_path, backed="r")
        try:
            observed = set(backed.obs["condition"].astype(str))
            valid = (
                backed.n_vars == len(var_names)
                and "CPA_pred" in backed.obsm
        ...
        if not valid:
            return None
        return metrics, inference_path, metrics_path
    except Exception as error:
        logger.warning(f"Existing shard for {cell_line} is incomplete: {error}")
        return None
...
def load_inference_data():
    ...
    try:
        ...
        adata_test = sc.read_h5ad(TAHOE_TEST_H5AD)
        adata_ctrl = sc.read_h5ad(TAHOE_CONTROL_H5AD)
...
#        adata_ctrl = sc.read_h5ad("../control_downsample.h5ad")
        ...
        # The corrected checkpoint stores cpa_split in its constructor metadata.
        # These inference-only labels reconstruct the model; they are never fitted.
        adata_all.obs["cpa_split"] = pd.Categorical(
            np.where(adata_all.obs["split"].eq("test"), "ood", "train"),
...
def perform_cell_line_batch_counterfactual_analysis(cpa_model, adata_all, global_control):
    ...
    os.makedirs("./results/random_inference_full_gauss", exist_ok=True)
    os.makedirs("./results/random_inference_full_gauss/by_cell_line", exist_ok=True)
    ...
    for cell_line_idx, (cell_line, conditions) in enumerate(conditions_by_cell_line.items(), 1):
        ...
        try:
            completed = load_completed_cell_line(
                cell_line, conditions, adata_all.var_names
            )
            if completed is not None:
                metrics, cell_line_path, metrics_path = completed
                ...
                logger.info(
                    f"Reused complete shard for {cell_line}: "
                    f"{len(metrics)} conditions"
                )
                continue
            ...
            if cell_line_results is None:
                ...
                failed_cell_lines.append(cell_line)
                continue
            ...
            cell_line_path = f"./results/random_inference_full_gauss/by_cell_line/{cell_line_filename}"
            temporary_cell_line_path = f"{cell_line_path}.tmp.h5ad"
            sc.write(temporary_cell_line_path, inference_adata)
            os.replace(temporary_cell_line_path, cell_line_path)

            # Save cell line specific metrics
            metrics_filename = f"metrics_{cell_line.replace('-', '_')}.json"
            metrics_path = f"./results/random_inference_full_gauss/by_cell_line/{metrics_filename}"
            temporary_metrics_path = f"{metrics_path}.tmp"
            with open(temporary_metrics_path, "w") as f:
                json.dump({
                    "cell_line": cell_line,
    ...
    if failed_cell_lines:
        raise RuntimeError(
            "Incomplete CPA inference; failed cell lines: "
            + ", ".join(sorted(set(failed_cell_lines)))
        )
    ...
    merged_metrics_path = "./results/random_inference_full_gauss/merged_metrics_by_condition.json"
    ...
    summary_path = "./results/random_inference_full_gauss/overall_summary_with_metrics.json"
    ...
    logger.info(f"  Cell line specific files in: ./results/random_inference_full_gauss/by_cell_line/")
...
def main():
    ...
    try:
        ...
        logger.info("  Main output directory: ./results/random_inference_full_gauss/by_cell_line/")
        logger.info("  Overall summary: ./results/random_inference_full_gauss/overall_summary_with_metrics.json")
        logger.info("  Merged metrics by condition: ./results/random_inference_full_gauss/merged_metrics_by_condition.json")
