...
- Full environment lock files from the original compute environment.
...
python -u examples/prepare_drug_embedding_tensor.py \
  ...
  --alias_json input_contracts/drug_name_aliases.json \
...
The PBMC example is documented in `examples/pbmc_ifnb/`. It records the exact stimulated-cell hold-out rule, provides a read-only input audit and links the manuscript-active configuration files to the full training entry point:
...
- The code snapshot is intended to support manuscript reproducibility and method inspection. Dataset access, preprocessing, and large checkpoints should be handled through the manuscript data/code availability statement or the public repository release.
- All command examples use repository-relative paths. Raw/processed H5ADs, checkpoints, latent tensors and other large objects are deliberately excluded; see `input_contracts/external_input_manifest.tsv` for staging names, sizes and hashes.
