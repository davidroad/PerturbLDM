...
BASE = Path("results/pipeline_results_gauss_random")
...
LATEST_PREDICTION_JSON = Path("results/random_inference_full_gauss/merged_metrics_by_condition.json")
LATEST_DISTRIBUTION_CSV = Path("results/random_distribution_similarity_gauss/global_condition_metrics.csv")
...
def main() -> None:
    ...
    if len(latest_prediction) != EXPECTED_CONDITIONS:
        raise AssertionError(
            f"latest CPA prediction conditions={len(latest_prediction)} "
            f"expected={EXPECTED_CONDITIONS}"
        )
    if len(latest_distribution) != EXPECTED_CONDITIONS:
        raise AssertionError(
            f"latest CPA distribution conditions={len(latest_distribution)} "
            f"expected={EXPECTED_CONDITIONS}"
        )
    ...
    if int(prediction_mask.sum()) != EXPECTED_CONDITIONS:
        raise AssertionError(
            f"benchmark CPA prediction rows={int(prediction_mask.sum())} "
            f"expected={EXPECTED_CONDITIONS}"
        )
    if not prediction_matches.all():
        raise AssertionError(
            f"unmatched CPA prediction rows={int((~prediction_matches).sum())}"
        )
    ...
    if int(distribution_mask.sum()) != EXPECTED_CONDITIONS:
        raise AssertionError(
            f"benchmark CPA distribution rows={int(distribution_mask.sum())} "
            f"expected={EXPECTED_CONDITIONS}"
        )
    if not distribution_matches.all():
        raise AssertionError(
            f"unmatched CPA distribution rows={int((~distribution_matches).sum())}"
        )
