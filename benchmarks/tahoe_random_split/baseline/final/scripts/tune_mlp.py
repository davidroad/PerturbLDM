#!/usr/bin/env python3
"""Full-train MLP dropout sensitivity without OOD/test access.

The frozen formal dropout=0 result is read only. Dropout 0.1 and 0.2 are
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=RUN / "sensitivity/mlp_full_train_dropout_search_20260727",
    )
    parser.add_argument(
        "--contract", type=Path, default=RUN / "config/hyperparameter_search.json"
    )
...
def main() -> None:
    ...
    table = pd.DataFrame(
        [row_for_comparison("formal_dropout_0p0", formal_row, validation_hash), *rows]
    ).sort_values(["validation_delta_mse", "dropout"], kind="stable").reset_index(drop=True)
    require(len(table) == 3, "comparison must contain dropout 0, 0.1 and 0.2")
    ...
    best = table.iloc[0]
    comparison_path = output_root / "full_train_dropout_comparison.csv"
    ...
    summary = {
        "status": "FULL_TRAIN_DROPOUT_SENSITIVITY_OK",
    ...
    write_json_atomic(output_root / "full_train_dropout_comparison.json", summary)
